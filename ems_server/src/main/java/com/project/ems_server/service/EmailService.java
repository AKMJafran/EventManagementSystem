...
public class EmailService {
    ...
    @Async
    public void sendLecturerWelcomeEmail(String email, String fullName, String temporaryPassword) {
        try {
            MimeMessage message = javaMailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true);
            ...
            helper.setFrom(fromEmail);
            helper.setTo(email);
            helper.setSubject(appName + " - Lecturer Account Created");

            String htmlContent = buildLecturerWelcomeEmail(fullName, email, temporaryPassword);
            helper.setText(htmlContent, true);

            javaMailSender.send(message);
        } catch (MessagingException | MailException e) {
            System.err.println("Failed to send lecturer welcome email: " + e.getMessage());
        }
    }
    ...
    private String buildLecturerWelcomeEmail(String fullName, String email, String temporaryPassword) {
        return "<!DOCTYPE html>" +
                "<html>" +
                "<head>" +
                "<style>" +
                ...
                "</div>" +
                "<p>Hello " + fullName + ",</p>" +
                "<p>Your lecturer account has been created by the Faculty administration.</p>" +
                "<div class='credential-box'>" +
                "<p><strong>Email:</strong> " + email + "</p>" +
                "<p><strong>Temporary Password:</strong> " + temporaryPassword + "</p>" +
                "</div>" +
