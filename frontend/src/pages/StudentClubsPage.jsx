      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 px-4 backdrop-blur-sm">
        <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
          <div className="flex items-start justify-between border-b border-slate-100 bg-slate-50/70 px-8 py-6">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">Register a New Club</h2>
              <p className="mt-2 max-w-xl text-sm text-slate-600">
                Your request will be reviewed by the Senior Treasurer then approved by the Dean.
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="overflow-y-auto px-8 py-8">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-5">
                <div className="mb-4">
                  <h3 className="text-base font-bold text-slate-900">Executive Committee</h3>
                  <p className="mt-1 text-sm text-slate-600">
                    Choose the student members who will serve as Secretary and Student Treasurer.
                  </p>
                </div>
              ...
              <div className="flex justify-end gap-3 border-t border-slate-100 pt-4">
                ...
                <button
                  type="submit"
                  disabled={submitting || lecturersLoading || studentsLoading || !!lecturersError || !!studentsError}
                  className="rounded-2xl bg-primary px-6 py-2.5 text-sm font-semibold text-white shadow-md transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60"
                >
...
export default function StudentClubsPage() {
  ...
  const lecturerById = useMemo(() => {
    ...
    for (const lecturer of lecturers) {
      map.set(lecturer.id, lecturer);
    }
    return map;
  }, [lecturers]);

  const availableStudents = useMemo(
    () => students.filter((student) => student.id !== currentUserId && student.isActive !== false),
  ...
  useEffect(() => {
    if (isModalOpen && lecturers.length === 0 && !lecturersLoading) {
      void fetchLecturers();
    }
  }, [isModalOpen, lecturers.length, lecturersLoading]);

  useEffect(() => {
    if (isModalOpen && students.length === 0 && !studentsLoading) {
      void fetchStudents();
    }
  }, [isModalOpen, students.length, studentsLoading]);
  ...
  const resolvedTreasurer = myClub ? lecturerById.get(myClub.seniorTreasurerLecturerId) : null;
  const treasurerMeta = resolvedTreasurer?.staffId || myClub?.seniorTreasurerStaffId || myClub?.seniorTreasurerLecturerEmail || 'Not available';
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm text-slate-600">
                  Register your club and bring students together. You will choose your executive committee before the request goes through Treasurer and Dean approval.
                </p>
              </div>
              <button
                type="button"
      ...
      {loading ? (
        ...
        clubs.length === 0 ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {clubs.map((club) => {
              const isOwnClub = myClub?.id === club.id;
                ...
                <article key={club.id} className="flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="mb-4 flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">Led by {club.presidentName || 'Student President'}</p>
                    </div>
                  ...
                  <div className="mb-6 space-y-2 text-sm text-slate-600">
                    <div className="flex items-center gap-2">
                      <span className="material-symbols-outlined text-[18px] text-slate-400">supervisor_account</span>
                      <span>Senior Treasurer: {club.seniorTreasurerLecturerName || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={joiningClubId === club.id || isOwnClub}
                  ...
                  >
                    {isOwnClub ? 'Your Club' : joiningClubId === club.id ? 'Joining...' : 'Join Club'}
                  </button>
                </article>
              );
            })}
        ...
        <div className="mx-auto max-w-5xl space-y-6">
          <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <p className="mt-4 text-sm leading-6 text-slate-600">
                  {myClub.description || 'No description available.'}
                </p>
              </div>
            </div>
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-3">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-2 text-sm text-slate-700">{myClub.description || 'No description provided.'}</p>
              </div>
            </div>
          ...
          {myClub.status === 'ACTIVE' && (
            <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
              <div className="mb-5">
                <h3 className="text-2xl font-bold text-slate-900">Club Members ({myClub.memberCount || 0})</h3>
                <p className="mt-1 text-sm text-slate-500">Your current club roster and membership roles.</p>
              </div>
        ...
        <div className="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-white px-8 py-14 text-center shadow-sm">
          ...
          <h2 className="text-2xl font-bold text-slate-900">You haven&apos;t registered a club yet</h2>
